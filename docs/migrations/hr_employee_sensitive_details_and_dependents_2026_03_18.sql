-- HR Employee Sensitive Details + Dependents + Contract Expiry + HR Manager flag
-- Adds strict RLS so sensitive fields are not accessible to unauthorized users.

create extension if not exists pgcrypto;

-- 1) HR Manager mapping on membership
alter table public.company_memberships
  add column if not exists is_hr_manager boolean not null default false;

-- 2) Contract expiry tracking (non-sensitive)
alter table public.hr_employees
  add column if not exists contract_expiry_date date null;

-- 3) Sensitive details (1:1)
create table if not exists public.hr_employee_sensitive_details (
  ...
  constraint hr_employee_sensitive_company_fk
    foreign key (company_id, employee_id) references public.hr_employees(company_id, id) on delete cascade
);
...
create index if not exists idx_hr_employee_dependents_company_employee on public.hr_employee_dependents(company_id, employee_id);
...
-- 7) RPC to replace dependents in one operation
