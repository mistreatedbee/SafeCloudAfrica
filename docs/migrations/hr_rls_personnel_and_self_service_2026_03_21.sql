...
drop policy if exists hr_employee_docs_select on public.hr_employee_documents;
...
drop policy if exists hr_employee_docs_insert on public.hr_employee_documents;
...
drop policy if exists hr_employee_docs_update on public.hr_employee_documents;
create policy hr_employee_docs_update on public.hr_employee_documents
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_employee_docs_delete on public.hr_employee_documents;
...
drop policy if exists hr_contracts_select on public.hr_employment_contracts;
create policy hr_contracts_select on public.hr_employment_contracts
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_contracts_insert on public.hr_employment_contracts;
create policy hr_contracts_insert on public.hr_employment_contracts
for insert
with check (public.is_platform_admin() or public.hr_has_personnel_access(company_id));

drop policy if exists hr_contracts_update on public.hr_employment_contracts;
create policy hr_contracts_update on public.hr_employment_contracts
for update
using (public.is_platform_admin() or public.hr_has_personnel_access(company_id))
with check (public.is_platform_admin() or public.hr_has_personnel_access(company_id));

drop policy if exists hr_contracts_delete on public.hr_employment_contracts;
...
drop policy if exists hr_timesheets_select on public.hr_timesheets;
create policy hr_timesheets_select on public.hr_timesheets
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_timesheets_insert on public.hr_timesheets;
...
drop policy if exists hr_timesheets_update on public.hr_timesheets;
create policy hr_timesheets_update on public.hr_timesheets
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_timesheets_delete on public.hr_timesheets;
...
drop policy if exists hr_disciplinary_select on public.hr_disciplinary_cases;
create policy hr_disciplinary_select on public.hr_disciplinary_cases
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_disciplinary_insert on public.hr_disciplinary_cases;
create policy hr_disciplinary_insert on public.hr_disciplinary_cases
for insert
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_disciplinary_update on public.hr_disciplinary_cases;
create policy hr_disciplinary_update on public.hr_disciplinary_cases
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_disciplinary_delete on public.hr_disciplinary_cases;
...
drop policy if exists hr_ack_select on public.hr_policy_acknowledgements;
create policy hr_ack_select on public.hr_policy_acknowledgements
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_insert on public.hr_policy_acknowledgements;
create policy hr_ack_insert on public.hr_policy_acknowledgements
for insert
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_update on public.hr_policy_acknowledgements;
create policy hr_ack_update on public.hr_policy_acknowledgements
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_delete on public.hr_policy_acknowledgements;
...
drop policy if exists hr_monthly_hours_select on public.hr_monthly_hours;
create policy hr_monthly_hours_select on public.hr_monthly_hours
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_monthly_hours_insert on public.hr_monthly_hours;
...
drop policy if exists hr_monthly_hours_update on public.hr_monthly_hours;
...
drop policy if exists hr_monthly_hours_delete on public.hr_monthly_hours;
...
drop policy if exists hr_ack_documents_insert on public.hr_ack_documents;
create policy hr_ack_documents_insert on public.hr_ack_documents
for insert
with check (public.is_platform_admin() or public.hr_has_personnel_access(company_id));

drop policy if exists hr_ack_documents_update on public.hr_ack_documents;
create policy hr_ack_documents_update on public.hr_ack_documents
for update
using (public.is_platform_admin() or public.hr_has_personnel_access(company_id))
with check (public.is_platform_admin() or public.hr_has_personnel_access(company_id));

drop policy if exists hr_ack_documents_delete on public.hr_ack_documents;
...
drop policy if exists hr_ack_receipts_select on public.hr_ack_receipts;
create policy hr_ack_receipts_select on public.hr_ack_receipts
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_receipts_insert on public.hr_ack_receipts;
create policy hr_ack_receipts_insert on public.hr_ack_receipts
for insert
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_receipts_update on public.hr_ack_receipts;
create policy hr_ack_receipts_update on public.hr_ack_receipts
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_ack_receipts_delete on public.hr_ack_receipts;
...
drop policy if exists hr_wellness_assessments_select on public.hr_employee_wellness_assessments;
create policy hr_wellness_assessments_select on public.hr_employee_wellness_assessments
for select
using (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_wellness_assessments_insert on public.hr_employee_wellness_assessments;
create policy hr_wellness_assessments_insert on public.hr_employee_wellness_assessments
for insert
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_wellness_assessments_update on public.hr_employee_wellness_assessments;
create policy hr_wellness_assessments_update on public.hr_employee_wellness_assessments
for update
using (public.hr_can_access_employee_row(company_id, employee_id))
with check (public.hr_can_access_employee_row(company_id, employee_id));

drop policy if exists hr_wellness_assessments_delete on public.hr_employee_wellness_assessments;
...
drop policy if exists hr_wellness_actions_select on public.hr_employee_wellness_actions;
...
drop policy if exists hr_wellness_actions_insert on public.hr_employee_wellness_actions;
...
drop policy if exists hr_wellness_actions_update on public.hr_employee_wellness_actions;
...
drop policy if exists hr_wellness_actions_delete on public.hr_employee_wellness_actions;
