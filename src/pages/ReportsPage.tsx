...
import { downloadCsvReport, downloadPdfReport, downloadWorkbook, type ReportRow } from '../api/services/reportExportService';
...
export function ReportsPage() {
  ...
  async function generate(template: ReportTemplate) {
    ...
    const withinDateRange = (value: unknown): boolean => {
    ...
    const downloadReport = async (reportId: ReportTemplate['id'], rows: ReportRow[]) => {
      const filenameBase = `${safeOrg}-${reportId}-${dateStamp}`;
      const title = `${template.name} - ${organisationName}`;
      let filename = `${filenameBase}.${exportFormat}`;
      if (exportFormat === 'xlsx') {
        downloadWorkbook(filename, [{ name: template.name, rows }]);
      } else if (exportFormat === 'pdf') {
        downloadPdfReport(filename, title, rows, metaLines);
      } else {
        downloadCsvReport(filename, rows, metaLines);
      }
      await createActivityLog({
        companyId: activeCompanyId,
        actorUserId: user.id,
        action: `reports.generate.${reportId}`,
        entityType: 'report',
        metadata: { filename, format: exportFormat.toUpperCase(), dateFrom: dateFrom || null, dateTo: dateTo || null, rowCount: rows.length }
      });
    };
    ...
    if (template.id === 'compliance') {
      ...
      ].filter((row) => withinDateRange(row.occurred_at));

      await downloadReport('compliance', rows);
      return;
    }
    ...
    if (template.id === 'incidents') {
      ...
      const rows = incidents
        .filter((i) => withinDateRange(i.occurred_at))
        .map((i) => ({
          id: i.id,
          module: i.module,
          category: i.category,
      ...
      await downloadReport('incidents', rows);
      return;
    }
    ...
    if (template.id === 'training') {
      ...
      const rows = records
        .filter((r) => withinDateRange(r.completed_at ?? (r as any).created_at))
        .map((r) => ({
          record_id: r.id,
          user_id: r.user_id,
      ...
      await downloadReport('training', rows);
      return;
    }
    ...
    if (template.id === 'audits') {
      ...
      const rows = allFindings
        .filter(({ finding, inspection }) => withinDateRange(finding.created_at ?? inspection.scheduled_at))
      ...
      await downloadReport('audits', rows);
      return;
    }
    ...
    if (template.id === 'inspections') {
      ...
      await downloadReport('inspections', rows.filter((row) => withinDateRange(row.scheduled_at ?? row.completed_at)));
      return;
    }
    ...
    if (template.id === 'pjo') {
      ...
      const rows = pjos.filter((pjo) => withinDateRange(pjo.observed_at)).flatMap((pjo) => {
        ...
        <motion.div variants={itemVariants}>
          ...
          <div className="mb-4 grid grid-cols-1 gap-3 rounded-xl border border-surface-300 bg-white p-4 md:grid-cols-3">
            <label className="text-sm">
              ...
              <input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} className="w-full rounded-lg border border-surface-300 px-3 py-2" />
            </label>
            <label className="text-sm">
              <span className="mb-1 block text-xs text-charcoal-500">To</span>
              <input type="date" value={dateTo} onChange={(event) => setDateTo(event.target.value)} className="w-full rounded-lg border border-surface-300 px-3 py-2" />
            </label>
            <label className="text-sm">
              <span className="mb-1 block text-xs text-charcoal-500">Export format</span>
              <select value={exportFormat} onChange={(event) => setExportFormat(event.target.value as ReportFormat)} className="w-full rounded-lg border border-surface-300 px-3 py-2">
