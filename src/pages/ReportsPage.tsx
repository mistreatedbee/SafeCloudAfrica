...
export function ReportsPage() {
  ...
  async function generate(template: ReportTemplate) {
    ...
    if (template.id === 'compliance') {
      ...
      const filename = `safecloudafrica-compliance-${dateStamp}.csv`;
    ...
    if (template.id === 'incidents') {
      ...
      const filename = `safecloudafrica-incidents-${dateStamp}.csv`;
      downloadTextFile(filename, csv, 'text/csv;charset=utf-8');
      await createActivityLog({
        companyId: activeCompanyId,
        actorUserId: user.id,
    ...
    if (template.id === 'training') {
      ...
      const filename = `safecloudafrica-training-${dateStamp}.csv`;
      downloadTextFile(filename, csv, 'text/csv;charset=utf-8');
      await createActivityLog({
        companyId: activeCompanyId,
        actorUserId: user.id,
    ...
    if (template.id === 'audits') {
      ...
      const filename = `safecloudafrica-audits-${dateStamp}.csv`;
      downloadTextFile(filename, csv, 'text/csv;charset=utf-8');
      await createActivityLog({
        companyId: activeCompanyId,
        actorUserId: user.id,
    ...
    const filename = `safecloudafrica-${template.id}-${dateStamp}.csv`;
    downloadTextFile(filename, toCsv([]), 'text/csv;charset=utf-8');
  }
