...
import { buildProfileLabelMap, formatUserProfileLabel, resolveUserLabel } from '../utils/userDisplayNames';
...
export function ImprovementDetailPage() {
  ...
  const userOptionLabel = (userId: string) =>
    formatUserProfileLabel(profiles?.find((p) => p.user_id === userId), userId);
      ...
      <div className="space-y-4">
        ...
        <section className="bg-white rounded-xl border border-surface-300 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <select value={form.raisedByUserId} onChange={(e) => setField('raisedByUserId', e.target.value)} className="px-3 py-2 border border-surface-300 rounded-lg">{userOptions.map((u) => <option key={u.user_id} value={u.user_id}>{userOptionLabel(u.user_id)}</option>)}</select>
          </div>
        ...
        <section className="bg-white rounded-xl border border-surface-300 p-4 space-y-2"><h3 className="font-semibold">SECTION 6: COMMENTS</h3><div className="space-y-1 max-h-36 overflow-auto">{(comments ?? []).map((c) => <div key={c.id} className="text-sm border border-surface-200 rounded p-2"><p className="text-xs text-charcoal-500">{new Date(c.created_at).toLocaleString('en-ZA')} • {resolveUserLabel(profileNameByUser, c.user_id)}</p>{c.message}</div>)}</div>{!isCreate && <><textarea rows={2} value={newComment} onChange={(e) => setNewComment(e.target.value)} className="w-full px-3 py-2 border border-surface-300 rounded-lg" /><button type="button" onClick={() => void postComment()} className="px-3 py-2 rounded-lg border border-surface-300 text-sm">Add Comment</button></>}</section>
