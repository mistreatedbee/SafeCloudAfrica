...
import { IMMEDIATE_CAUSES_UNSAFE_ACTS_GROUPS } from '../../api/models/core';
...
export function IncidentDetailModal(props: {
  ...
  const immediateCauseCategoryOptions = useMemo(() => Object.keys(IMMEDIATE_CAUSES_UNSAFE_ACTS_GROUPS), []);
  const immediateCauseSubcategoryOptions = useMemo(() => {
    const key = immediateCauseCategoryInput.trim() as keyof typeof IMMEDIATE_CAUSES_UNSAFE_ACTS_GROUPS;
    const list = IMMEDIATE_CAUSES_UNSAFE_ACTS_GROUPS[key];
    return Array.isArray(list) ? Array.from(list) : [];
  }, [immediateCauseCategoryInput]);
                      ...
                      <div className="lg:col-span-2 rounded-lg border border-surface-200 p-3 bg-surface-50">
                        ...
                        <div className="mt-3 grid grid-cols-1 lg:grid-cols-3 gap-3">
                          <div>
                            ...
                            <input value={immediateCauseCategoryInput} onChange={(e) => setImmediateCauseCategoryInput(e.target.value)} list="immediate-cause-parent-options" disabled={!props.canEditInvestigation} placeholder="e.g. Unsafe Act" className="w-full px-3 py-2 bg-white border border-surface-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal disabled:opacity-60" />
                            <datalist id="immediate-cause-parent-options">
                              {immediateCauseCategoryOptions.map((entry) => (
                          ...
                          <div>
                            ...
                            <input value={immediateCauseSubcategoryInput} onChange={(e) => setImmediateCauseSubcategoryInput(e.target.value)} list="immediate-cause-subcategory-options" disabled={!props.canEditInvestigation} placeholder="e.g. Improper lifting" className="w-full px-3 py-2 bg-white border border-surface-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal disabled:opacity-60" />
                            <datalist id="immediate-cause-subcategory-options">
                              {immediateCauseSubcategoryOptions.map((entry) => (
                                <option key={entry} value={entry} />
                              ))}
                            </datalist>
                          </div>
