...
export function KpiCreateModal(props: {
  ...
  const [status, setStatus] = useState<PlanningKpi['status']>('on-track');
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-xl mx-4 bg-white rounded-2xl shadow-xl border border-surface-200 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-surface-200">
          <p className="text-sm font-semibold text-charcoal">Add KPI</p>
        ...
        <form onSubmit={onSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Monthly training compliance"
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as PlanningKpi['status'])}
              className="w-full px-4 py-2.5 bg-white border border-surface-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal focus:border-transparent"
            >
