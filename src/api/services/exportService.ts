...
export function exportPpeIssueRegisterCSV(issues: Array<Record<string, unknown>>): Blob {
  const headers = ['Issue Date', 'PPE Item', 'Category', 'Quantity', 'Issued To', 'Department', 'Site', 'Total Cost'];
  const rows = issues.map((issue) => [
    String(issue.issue_date ?? ''),
    String(issue.ppe_item_name ?? ''),
    String(issue.ppe_category ?? ''),
    String(issue.quantity_issued ?? ''),
    String(issue.issued_to_name ?? issue.issued_to_user_id ?? ''),
    String(issue.department_name ?? issue.department_id ?? ''),
    String(issue.site_name ?? issue.site_id ?? ''),
    String(issue.total_cost_at_issue ?? '')
  ]);
  const csv = [headers.join(','), ...rows.map((row) => row.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(','))].join('\n');
  return new Blob([csv], { type: 'text/csv;charset=utf-8;' });
}

export function exportPpeCostSummaryCSV(summary: any): Blob {
  const rows = [
    ['Total PPE Cost', String(summary?.totalPpeCost ?? 0)],
    [''],
    ['Top Costing Items', '', '', ''],
    ['Item', 'Total Cost', 'Quantity Issued', 'Avg Cost/Issue'],
    ...((summary?.topCostingPpeItems ?? []).map((item: any) => [
      item.ppeItemName ?? item.ppeItemId ?? '',
      String(item.totalCost ?? 0),
      String(item.quantityIssued ?? 0),
      String(item.avgCostPerIssue ?? 0)
    ]) as string[][])
  ];
...
export function exportPpeUsageSummaryCSV(summary: any): Blob {
  const rows = [
    ['Total Quantity Issued', String(summary?.totalQuantityIssued ?? 0)],
    [''],
    ['Usage by PPE Item', 'Quantity'],
    ...((summary?.usageByPpeItem ?? []).map((item: any) => [item.ppeItemName ?? item.ppeItemId ?? '', String(item.quantityIssued ?? 0)]) as string[][]),
    [''],
    ['Reason Breakdown', 'Count', 'Quantity'],
    ...((summary?.reasonBreakdown ?? []).map((row: any) => [row.reason ?? 'Not specified', String(row.count ?? 0), String(row.quantity ?? 0)]) as string[][])
  ];
  const csv = rows.map((row) => row.map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(',')).join('\n');
  return new Blob([csv], { type: 'text/csv;charset=utf-8;' });
}

export async function exportPpeCostSummaryPDF(summary: any): Promise<Blob> {
  const html = `
    <html><body>
      <h1>PPE Cost Summary</h1>
      <p>Total PPE Cost: ${summary?.totalPpeCost ?? 0}</p>
      <table border="1" cellspacing="0" cellpadding="6">
        <thead><tr><th>Item</th><th>Total Cost</th><th>Qty</th><th>Avg/Issue</th></tr></thead>
        <tbody>
          ${(summary?.topCostingPpeItems ?? [])
            .map(
              (item: any) =>
                `<tr><td>${item.ppeItemName ?? item.ppeItemId ?? ''}</td><td>${item.totalCost ?? 0}</td><td>${item.quantityIssued ?? 0}</td><td>${item.avgCostPerIssue ?? 0}</td></tr>`
            )
            .join('')}
        </tbody>
      </table>
...
export async function exportPpeUsageSummaryPDF(summary: any): Promise<Blob> {
  const html = `
    <html><body>
      <h1>PPE Usage Summary</h1>
      <p>Total Quantity Issued: ${summary?.totalQuantityIssued ?? 0}</p>
      <table border="1" cellspacing="0" cellpadding="6">
        <thead><tr><th>Item</th><th>Quantity</th></tr></thead>
        <tbody>
          ${(summary?.usageByPpeItem ?? [])
            .map((item: any) => `<tr><td>${item.ppeItemName ?? item.ppeItemId ?? ''}</td><td>${item.quantityIssued ?? 0}</td></tr>`)
            .join('')}
        </tbody>
      </table>
    </body></html>
