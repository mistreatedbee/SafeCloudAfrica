...
export async function upsertEnvWaterMonitoring(input: any): Promise<any> {
  ...
  const payload = {
    ...
    monitoring_process: input.monitoringProcess?.trim() || null,
    ...
    blocked_drains_or_overflows: input.blockedDrainsOrOverflows?.trim() || null,
    equipment_used: (input.equipmentUsed ?? []).map((x: any) => String(x).trim()).filter(Boolean),
    photo_file_ids: input.photoFileIds ?? [], parameters,
